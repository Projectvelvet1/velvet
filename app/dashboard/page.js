...
export default function Dashboard() {
  ...
  const [clients, setClients] = useState([]);   // agency view
  const [workspace, setWorkspace] = useState(null); // client view
  const [services, setServices] = useState([]);      // client view
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: prof } = await supabase
        .from("profiles").select("full_name,email,side,is_super_admin").eq("id", session.user.id).single();
      const p = prof || { email: session.user.email, side: "agency" };
      setProfile(p);

      if (p.side === "client") {
        const { data: ws } = await supabase
          .from("workspaces").select("id,name,onboarding_complete").limit(1);
        const w = ws?.[0] || null;
        setWorkspace(w);
        if (w) {
          const { data: svc } = await supabase
            .from("client_services").select("department,service_label,service_key").eq("workspace_id", w.id);
          setServices(svc || []);
        }
      } else {
        const { data: ws } = await supabase
          .from("workspaces").select("id,name,is_demo,onboarding_complete").order("created_at", { ascending: true });
        setClients(ws || []);
      }
      setLoading(false);
    })();
  ...
  // group client services by department
  const grouped = {};
  services.forEach((s) => { (grouped[s.department] ||= []).push(s); });
            ...
            {clients.length === 0 ? (
              ...
              <div className="card" key={w.id}>
                ...
                <div style={{ color: "var(--muted)", fontSize: 13, marginTop: 4 }}>
                  {w.onboarding_complete ? "Onboarding complete" : "Waiting onboarding"}
                </div>
              </div>
            ))}
          </>
          ...
          !workspace ? (
            ...
            <>
              ...
              <p style={{ color: "var(--faint)", fontSize: 13, marginTop: 0 }}>
                These unlock once onboarding is complete.
              </p>
              {services.length === 0
                ? <div className="empty">Your services will appear here.</div>
                : services.map((s) => (
                  <div className="card" key={s.service_key} style={{ opacity: .55 }}>
                    <b>{s.service_label}</b>
            ...
            <>
              ...
              <h3 style={{ fontSize: 16, margin: "18px 0 10px" }}>Your services</h3>
              {services.map((s) => (
                <div className="card" key={s.service_key} style={{ cursor: "pointer" }}>
                  <b>{s.service_label}</b>
