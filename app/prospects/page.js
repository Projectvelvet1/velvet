...
export default function Prospects() {
  ...
  async function add(e) {
    ...
    if (!res.ok) { setErr(j.error || "Could not add future client"); return; }
  ...
  return (
    <Shell profile={profile} roleLabel="Super admin" nav={nav}>
      <div className="page-head"><h1 style={{ fontSize: 24 }}>Future Clients</h1>
        {allowed && <button className="btn btn-primary" onClick={openModal}>+ Add future client</button>}
      </div>
      ...
      {!allowed ? <div className="empty">Super admins only.</div>
        : prospects.length === 0 ? <div className="empty">No future clients yet. Click "Add future client" to invite your first one.</div>
        : prospects.map((p) => (
          <div className="card" key={p.id}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10 }}>
              <b>{p.name}</b><span className="pill p-agency">future client</span>
            </div>
      ...
      {show && (
        <Modal title="Add a future client" onClose={() => setShow(false)}>
          ...
          {!link ? (
            ...
            <>
              <div className="auth-msg auth-ok">Invited. Share this sign-in link with them too:</div>
              <div className="field">
      ...
      {delPros && (
        <ConfirmDelete
          title={`Delete future client ${delPros.name}`}
