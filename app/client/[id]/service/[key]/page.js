...
export default function ClientServiceDashboard() {
  ...
  return (
    <Shell profile={profile} roleLabel={isClient ? "Client" : (profile?.is_super_admin ? "Super admin" : "Team member")} nav={isClient ? clientNav : <AgencyNav profile={profile} active={"svc:" + key} depts={depts} />}>
      ...
      {!isSeo && !isPaid && !isAso && !isDashboarding && !isTracking && <div className="card" style={{ marginTop: 4 }}><b>{svc?.label} dashboard</b><div style={{ fontSize: 13, color: "var(--faint)", marginTop: 4 }}>This service's dashboard is coming soon. Tasks and documents below are already active.</div></div>}
      {isSeo && (<>
